...
class ProfilePictureService {
  ...
  async uploadProfilePicture(file: File): Promise<string> {
    ...
    const { uploadUrl, fileKey } = await this.generateUploadUrl(fileExtension);

    // Upload file to S3
    await this.uploadFile(file, uploadUrl);

    // Confirm upload and update profile - use the confirmed URL from backend
    const { profilePictureUrl } = await this.confirmUpload(fileKey);
