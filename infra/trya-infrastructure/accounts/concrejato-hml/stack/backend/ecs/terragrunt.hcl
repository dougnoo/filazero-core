...
inputs = merge(
  local.common_vars,
  {
    ...
    task_name = "${local.account_vars.locals.account_name}-backend-task"
    ...
    container_name  = "${local.account_vars.locals.account_name}-backend"
    
    target_group_arn = dependency.alb.outputs.target_group_arn
    
    # Security Groups
    alb_security_group_id = dependency.alb.outputs.security_group_id
    aurora_security_group_ids = []  # Será preenchido após Aurora ser criado
    elasticache_security_group_ids = []  # Será preenchido após ElastiCache ser criado
    ...
    environment_variables = [
      ...
      { name = "POSTGRES_HOST", value = "${local.account_vars.locals.account_name}-aurora.cluster-xyz.us-east-1.rds.amazonaws.com" },
      { name = "POSTGRES_PORT", value = "5432" },
      { name = "POSTGRES_DB", value = "trya" },
      { name = "REDIS_HOST", value = "${local.account_vars.locals.account_name}-cache.cache.amazonaws.com" },
    ...
    container_health_check = {
      command = [
        "CMD-SHELL",
        "curl -f http://localhost:${local.common_vars.container_port}${local.common_vars.health_check_path} || exit 1"
      ]
      interval    = 30
      timeout     = 5
      retries     = 3
