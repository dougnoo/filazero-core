...
# Platform Stack
# ==============================================================================
# Provisions: ECR, ECS Service, ALB, S3 (for assets), Route53 record
# for the platform backend (handslab-trya-platform-backend).
#
...
locals {
  ...
  # Platform domain based on environment
  # dev: platform-dev.trya.com.br
  # hml: platform-hml.trya.com.br
  # prod: platform.trya.com.br
...
module "s3_assets" {
  ...
  project_name = var.project_name
  environment  = var.environment
  bucket_name  = "${lower(var.project_name)}-${var.environment}-platform-assets"
...
resource "aws_security_group" "ecs_tasks" {
  name_prefix = "${local.name_prefix}-platform-ecs-"
  description = "Security group for platform ECS tasks"
  vpc_id      = local.vpc_id
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "${local.name_prefix}-platform-ecs-sg"
    }
  )
...
module "ecs_service" {
  ...
  container_image = var.container_image != "" ? var.container_image : "${module.ecr.repository_urls["platform"]}:latest"
  ...
  environment_variables = merge(
    {
      ...
      AWS_S3_BUCKET_NAME = module.s3_assets.bucket_id
    },
