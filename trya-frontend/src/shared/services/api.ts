...
const getSelectedTenantId = (): string | null => {
  if (typeof window === "undefined") return null;
  try {
    const fromStorage = localStorage.getItem(SUPER_ADMIN_TENANT_ID_STORAGE_KEY);
  ...
  // fallback: permite passar tenantId via query param
  try {
    const tenantIdFromQuery = new URLSearchParams(window.location.search).get(
      "tenantId",
    );
...
const request = async <T = unknown>(
  ...
  // SUPER_ADMIN (sem tenant no token) precisa informar o tenant atual via header
  // Para roles normais, o backend prioriza o tenant do usuário autenticado, então este header é ignorado.
  const selectedTenantId = getSelectedTenantId();
  if (!isNoAuthenticationEndpoint && selectedTenantId) {
