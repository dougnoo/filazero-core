...
export class GetMunicipalitiesUseCase {
  ...
  async execute(userId: string, state: string): Promise<string[]> {
    if (!userId) {
    ...
    const provider = await this.networkProviderRepository.getProviderAndPlanNameByUserId(
      userId,
    );
    const municipalities = await this.networkProviderApiRepository.getMunicipalitiesByState(
      state,
